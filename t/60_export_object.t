...
# $Id: 60_export_object.t,v 1.3 2004/06/12 18:38:56 lachoy Exp $
...

{
    ...
    is( $export_all_data, $ALL,
        'Export all data matches (no ID)' );

    $exporter->where( "myname = 'bar'" );
    $export_some_data = eval { $exporter->run };
    is( $exporter->number_exported, 1,
        'Correct number of records counted with some data (no ID)' );
    ...
    is( $export_all_id_data, $ALL_ID,
        'Export all data matches (with ID)' );
    ...
    is( $exporter_id->number_exported, 1,
        'Correct number of records counted with some data (with ID)' );
    is( $export_some_id_data, $SOME_ID,
        'Export some data matches (with ID)' );
}

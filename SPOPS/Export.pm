...
# $Id: Export.pm,v 3.7 2004/06/12 19:14:52 lachoy Exp $

use strict;
use base qw( Class::Accessor Class::Factory );
use SPOPS::Exception qw( spops_error );

$SPOPS::Export::VERSION  = sprintf("%d.%02d", q$Revision: 3.7 $ =~ /(\d+)\.(\d+)/);

use constant AKEY => '_attrib';

my @FIELDS = qw(
    object_class where value include_id skip_security
    skip_fields number_exported DEBUG
);
...
sub run {
    ...
    $self->number_exported( $record_count );
...
Set to true if you want to skip security checks for the fetch.

Default: false, security will be checked.
...
After C<run()> is executed this returns the number of records
exported. If called before C<run()> it will just return 0.
